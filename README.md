...
The inequality differs between these two notations. Little-o has the inequality $\( < \)$, indicating that $f(n)\$ grows slower than $\ g(n) \$. Big-O has the inequality $\( \leq \)$, indicating that $f(n)\$ grows at most as fast as $\ g(n) \$. Therefore, $f(n) < cg(n)\$ implies $\ f(n) \leq cg(n) \$.
